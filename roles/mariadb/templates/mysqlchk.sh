...
MYSQL_PORT="3306"
MYSQL_USERNAME='{{ mariadb_info.rpluser }}'
MYSQL_PASSWORD='{{ mariadb_info.rpluser_password }}'
...
#    MASTER_SLAVE_HOSTS=$($CMDLINE 'SHOW SLAVE HOSTS' 2>/dev/null | wc -l)
    SLAVE_THREADS_COUNT=$($CMDLINE 'SELECT COUNT(1) SlaveThreadCount FROM information_schema.processlist WHERE user="system user";' --vertical 2>/dev/null | tail -1 | awk {'print $2'})

#    if [ $MASTER_SLAVE_HOSTS -ge 1 ]; then
    if [ $SLAVE_THREADS_COUNT -eq 0 ]; then
        role='master'
